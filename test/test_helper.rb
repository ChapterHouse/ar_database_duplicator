...
#require File.expand_path(File.dirname(__FILE__) + '/lib/time_assertions')
#require File.expand_path(File.dirname(__FILE__) + '/lib/string_assertions')
=begin
class ActiveSupport::TestCase
  ...
  include DailyDealRevenueReportsTestHelper
  include TravelsaversTestHelper
  include PurchaseSessionTestHelper
  include Testing::HTMLAssertions
  include PasswordPolicyTestHelper
  include Testing::TimeAssertions
  include Testing::StringAssertions
  ...
  def assert_i18n_message(key, message, options = {})

    options[:locale] ||= :en
    if options[:theme]
      string = Analog::Themes::I18n.translate(options[:theme], key)
    else
      string = I18n.t(key.to_sym, :locale => options[:locale])
    end
    string.gsub!(/%\{[\w]+\}/, '*')

    escaped = Regexp.escape(string).gsub('\*','.*?')
    string_regex = Regexp.new "^#{escaped}$", Regexp::IGNORECASE

    if message.is_a? Array
      assert_match string_regex, message.first
    else
      assert_match string_regex, message
    end
  end
  # assert_localised_error :invalid_password, json["errors"], {:}
  ...
  def login_with_basic_auth(options = {})
    ...
    when User
      @request.env['HTTP_AUTHORIZATION'] = 'Basic ' + Base64::encode64("#{options.login}:#{PasswordPolicyTestHelper.default_test_password}")
    else
      raise ArgumentError, "expected Hash or User object, got #{options}"
    end
  end

  def login_with_publisher_session(consumer, password)
    post publisher_daily_deal_sessions_path(consumer.publisher), :session => { :email => consumer.login, :password => password }
  end
...
ActionController::IntegrationTest.class_eval do
  ...
  def daily_deal_logout(publisher)
    get daily_deal_logout_path(publisher.to_param)
  end
  ...
  def assert_logged_out_from_publisher(publisher)
